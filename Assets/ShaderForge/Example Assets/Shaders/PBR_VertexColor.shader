...
///////// Gloss:
                ...
                float gloss = 1.0 - (_Roughness_var*_RGB_var.g); // Convert roughness to gloss
                float perceptualRoughness = (_Roughness_var*_RGB_var.g);
...
////// Specular:
                ...
                float3 specularColor = (_RGB_var.r*_Metallic_var);
                ...
                float3 diffuseColor = lerp(node_3278,dot(node_3278,float3(0.3,0.59,0.11)),_Desaturation_var); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 node_3278 = (_MainTex_var.rgb*i.vertexColor.rgb);
                float _Desaturation_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Desaturation );
                float3 diffColor = lerp(node_3278,dot(node_3278,float3(0.3,0.59,0.11)),_Desaturation_var);
                float specularMonochrome;
                float3 specColor;
                float4 _RGB_var = tex2D(_RGB,TRANSFORM_TEX(i.uv0, _RGB));
                float _Metallic_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metallic );
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, (_RGB_var.r*_Metallic_var), specColor, specularMonochrome );
                float _Roughness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Roughness );
                float roughness = (_Roughness_var*_RGB_var.g);
